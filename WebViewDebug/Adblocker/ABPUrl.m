...
@implementation ABPUrl {

}

//+ (NSString *)getDecodedHostname:(NSURL *)url {
//    NSString *hostName = url.host;
//    if ([hostName indexOfString:@"xn--"] == -1) {
//        return hostName;
//    }
//    return hostName;
//}
...
+ (BOOL)isDomain:(NSString *)hostName {
    NSString *pattern = @"^((0x[\\da-f]+|\\d+)(\\.|$))*$";
    NSRegularExpression *regex = [NSRegularExpression regularExpressionWithPattern:pattern options:NSRegularExpressionCaseInsensitive error:nil];
    NSTextCheckingResult *result = [regex firstMatchInString:hostName options:0 range:NSMakeRange(0, hostName.length)];
    if (result) {
        return false;
    }
    return [hostName indexOfString:@":"] == -1;
}

//+ (NSString *)getBaseDomain:(NSString *)hostName {
//    NSArray *bits = [hostName split:@".".regExp];
//    NSInteger cutoff = bits.count - 2;
//    for (int i = 0; i < bits.count; i++) {
//        NSString *key = [[bits subarrayWithRange:NSMakeRange(i, bits.count - i)] componentsJoinedByString:@"."];
//        NSNumber *offsetValue = [ABPUrl publicSuffixes][key];
//        if(offsetValue){
//            cutoff = i - offsetValue.intValue;
//            break;
//        }
//    }
//
...
//    }
//
//    return [[bits subarrayWithRange:NSMakeRange((NSUInteger) cutoff, bits.count - cutoff)] componentsJoinedByString:@"."];
//}

+(BOOL)isThirdPart:(NSString *)reqHost documentHost:(NSString *)documentHost{
    NSString *requestHost = [reqHost replace:RX(@"\\.+$") with:@""];
    documentHost = [documentHost replace:RX(@"\\.+$") with:@""];

    if(requestHost == documentHost){
        return false;
    }

    if(![ABPUrl isDomain:requestHost] || ![ABPUrl isDomain:documentHost]){
        return true;
    }

    return ![requestHost isEqualToString:documentHost];
}

+ (NSString *)typeMaskOfUrl:(NSString *)urlString {
    if([urlString isMatch:RX(@"\\.png|\\.jpg|\\.gif|\\.jpeg|\\.bmp")]){
        return @"IMAGE";
    }else if([urlString isMatch:RX(@"\\.css")]){
        return @"STYLESHEET";
    }else if([urlString isMatch:RX(@"\\.js")]){
        return @"SCRIPT";
    }else{
        return @"OTHER";
    }
